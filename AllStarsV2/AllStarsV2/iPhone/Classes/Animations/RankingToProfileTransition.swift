//
//  RankingToProfileTransition.swift
//  AllStarsV2
//
//  Created by Kenyi Rodriguez Vergara on 21/09/17.
//  Copyright © 2017 Kenyi Rodriguez Vergara. All rights reserved.
//

import UIKit

class RankingToProfileInteractiveTransition : InteractiveTransition{
...
class RankingToProfileTransition: ControllerTransition {

    override func createInteractiveTransition(navigationController: UINavigationController) -> InteractiveTransition? {
        
        let interactiveTransition = RankingToProfileInteractiveTransition()
    ...
    override func animatePush(toContext context : UIViewControllerContextTransitioning){
        ...
        let fromVC = self.controllerOrigin as! RankingViewController
        ...
        let relativeFrame = fromVC.clvUsers.convert(cellSelected.frame, to: containerView)
    ...
    override func animatePop(toContext context : UIViewControllerContextTransitioning) {
        ...
        let toVC   = self.controllerDestination as! RankingViewController
        ...
        UIView.animate(withDuration: duration, delay: 0, usingSpringWithDamping: 0.8, initialSpringVelocity: 0.1, options: .curveEaseIn, animations: {
            ...
            imgUser.transform = CGAffineTransform(scaleX: delta, y: delta)
            var relativeFrame = toVC.clvUsers.convert(cellSelected.frame, to: containerView)
