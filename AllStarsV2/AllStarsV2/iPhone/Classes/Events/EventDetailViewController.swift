...
class EventDetailViewController: SWFrontGenericoViewController, UIScrollViewDelegate {
    ...
    @IBOutlet weak var eventInformationBackgroundView : UIView!
    @IBOutlet weak var buttonsSectionView           : UIView!
    ...
    @IBOutlet weak var buttonsSectionsViewTopConstraint     : NSLayoutConstraint!
    @IBOutlet weak var eventTitleLabelTopConstraint         : NSLayoutConstraint!
    ...
    /* @IBAction func clickBtnRegister(_ sender: Any) {
        EventBC.updateUserToEvent(toEvent: self.objEvent, withSuccessful: { (obj) in
            ...
            CDMUserAlerts.showSimpleAlert(title: "Success".localized,
                                          withMessage: message,
    ...
    func transitionViewSectionInto(_ scrollView : UIScrollView){
        if Int(scrollView.contentOffset.x) % Int(scrollView.frame.size.width) != 0 {
            ...
            let alpha = (index < self.currentIndexSection) ? (CGFloat(self.currentIndexSection) - delta) : (delta - CGFloat(self.currentIndexSection))
